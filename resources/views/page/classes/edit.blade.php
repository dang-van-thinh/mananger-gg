...
@section('content')
    <div class="card">
        <h3 class="card-header text-center">Cập nhật lớp học</h3>
        <div class="p-4">
            <form action="{{ route('classes.update', $class) }}" method="post" id="formHandler"
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="name" class="form-control" id="name"
                                    value="{{ old('name', $class->name) }}" placeholder="Nhập tên lớp học">
                            </div>
                            @if ($errors->has('name'))
                            ...
                            <div class="input-group">
                                ...
                                <select name="course_id" id="course_id" class="form-select">
                                    @foreach ($data['courses'] as $course)
                                        <option @selected($class->course_id == $course->id) value=" {{ $course->id }} ">
                                            {{ $course->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="room_id" id="room_id" class="form-select">
                                    @foreach ($data['rooms'] as $room)
                                        <option @selected($class->room_id == $room->id) value=" {{ $room->id }} ">
                                            {{ $room->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <select name="teacher_id" id="teacher_id" class="form-select">
                                    @foreach ($data['teachers'] as $teacher)
                                        <option @selected($class->teacher_id == $teacher->id) value=" {{ $teacher->id }} ">
                                            {{ $teacher->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                <select name="day_of_week_id[]" id="day_of_week_id" class="form-select" multiple>
                                    ...
                                    @foreach ($data['dayOfWeeks'] as $day)
                                        <option @if (in_array($day->id, $dayOfClassId->toArray())) selected @endif
                                            value=" {{ $day->id }} ">
                                            {{ $day->name }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="input-group">
                                ...
                                <select name="session_id" id="session_id" class="form-select">
                                    ...
                                    @foreach ($data['sessions'] as $session)
                                        <option @selected($class->session_id == $session->id) class="opt-session"
                                            value=" {{ $session->id }} ">
                                            {{ $session->name }}
                                        </option>
                                    @endforeach
